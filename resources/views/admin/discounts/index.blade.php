...
@section('content')
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <h1 class="h4 mb-0 text-gray-800"> تخفیف ها </h1>
        <a href="{{ route('admin.discounts.create') }}" class="btn btn-sm btn-success @cannot('discounts-create') disabled @endcannot">
        ...
        <form action="" id="search_form" class="col-sm-8 col-md-6 col-lg-4 mr-auto my-2 navbar-search">
            <div class="input-group">
                ...
                <input type="text" class="form-control bg-white border-0 small" name='search'
                    value="{{ request()->get('search') }}" placeholder="نام تخفیف " aria-label="Search"
    ...
    <div id="pagination_data" class="table-responsive">
        @include('admin.discounts.discounts-paginate', ['discounts' => $discounts])
    </div>
    {{-- DeleteModal --}}
        ...
        <div class="modal-dialog modal-dialog-scrollable modal-sm">
            <div class="modal-content">
                <div class="modal-body">
                    ...
                    <form class="mt-3" action="{{ route('admin.discounts.destroy') }}" method="post" id="delete_from">
                        @csrf
                        @method('delete')
    ...
    <script>
        $(document).ready(function() {
            $(document).on('click', 'a[data-target="#delete_modal"]', function() {
                $('input[name="discount_id"]').val($(this).attr('data-id'));
            });
        });
    </script>
@endsection
